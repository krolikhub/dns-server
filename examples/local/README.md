...
- **[QUICKSTART.md](QUICKSTART.md)** - Быстрый старт за 5 минут (включая решение проблем с SSH)
- **[TESTING.md](TESTING.md)** - Подробное руководство по тестированию и отладке
- **[test-dns-server.sh](test-dns-server.sh)** - Автоматический скрипт проверки всех компонентов

**Рекомендуется:** Начните с [QUICKSTART.md](QUICKSTART.md) для быстрого запуска!
