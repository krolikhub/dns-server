...
timeout 5 ssh -i ~/.ssh/id_ed25519 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=3 ubuntu@${DNS_IP} "echo 'SSH OK'" > /dev/null 2>&1
check_test $?
...
if [ $FAILED -eq 0 ]; then
    ...
    echo -e "  • SSH доступ:    ${YELLOW}ssh -i ~/.ssh/id_ed25519 ubuntu@${DNS_IP}${NC}"
    ...
    echo -e "  2. Проверьте SSH:      ${YELLOW}ssh -i ~/.ssh/id_ed25519 ubuntu@${DNS_IP}${NC}"
    echo -e "  3. Проверьте сервисы:  ${YELLOW}ssh -i ~/.ssh/id_ed25519 ubuntu@${DNS_IP} 'systemctl status pdns'${NC}"
    exit 1
fi
