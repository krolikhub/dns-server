...
## Проблема: SSH подключение по ключу не работает
...
### Признаки
- SSH сервер доступен (порт 22 открыт)
- VM пингуется
- SSH клиент предлагает ключ, но сервер его отвергает
- В debug выводе видно: `Authentications that can continue: publickey`

### Причина
Cloud-init не добавил SSH ключ в `/home/ubuntu/.ssh/authorized_keys` или:
- Файл имеет неправильные права доступа
- Ключ добавлен неправильно (ошибки форматирования)
- Директория `.ssh` не существует или имеет неправильные права
...
```bash
# Запустите скрипт исправления SSH ключей
bash scripts/fix-ssh-keys.sh
```

Скрипт автоматически:
- Подключится к VM по паролю (пароль по умолчанию: `ubuntu`)
- Создаст директорию `.ssh` с правильными правами
- Добавит ваш публичный ключ в `authorized_keys`
- Установит правильные права доступа (700 для директории, 600 для файлов)
- Проверит результат

#### Вариант 2: Ручное исправление

Подробная документация: [docs/SSH_KEY_FIX_RU.md](docs/SSH_KEY_FIX_RU.md)

```bash
# 1. Подключитесь к VM через консоль
...
# (Скопируйте содержимое вашего ~/.ssh/id_rsa.pub)
...
Для полной диагностики проблемы:
```bash
bash scripts/debug-ssh-full.sh
```

Подробная информация: [docs/SSH_KEY_FIX_RU.md](docs/SSH_KEY_FIX_RU.md)
