...
### 0. Проверка предварительных требований

Перед началом установки проверьте все зависимости:

```bash
# Запустить проверку всех требований
./scripts/check-prerequisites.sh
```

Скрипт проверит наличие всех необходимых инструментов и подскажет, что нужно установить.

### 1. Установка зависимостей

⚠️ **ВАЖНО**: Пакет `xsltproc` является критически важным! Без него возникнет ошибка "Permission denied" при создании VM.
...
- **xsltproc** (⚠️ **КРИТИЧНО** - требуется для libvirt провайдера, без него возникает Permission denied)
- Linux хост с KVM поддержкой

Для автоматической проверки всех требований запустите:
```bash
./scripts/check-prerequisites.sh
```
